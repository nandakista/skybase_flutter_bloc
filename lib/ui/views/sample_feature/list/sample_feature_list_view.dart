  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            BlocConsumer<NandaBloc, SampleFeatureListState>(
              listener: (BuildContext context, SampleFeatureListState state) {
                NandaBloc bloc = context.read<NandaBloc>();
            ...
            BlocConsumer<PermanaBloc, SampleFeatureListState>(
              listener: (BuildContext context, SampleFeatureListState state) {
                PermanaBloc bloc = context.read<PermanaBloc>();
                if (state is SampleFeatureListError) {
                  bloc.loadError(state.message);
                } else if (state is SampleFeatureListLoaded) {
                  bloc.loadNextData(data: state.result);
